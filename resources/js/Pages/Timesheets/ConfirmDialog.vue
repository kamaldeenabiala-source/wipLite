...
// On récupère le rôle pour adapter le texte
    ...
    >
        <div class="flex flex-col items-center p-2 text-center">
            ...
            <p class="text-sm text-gray-500 mb-6 px-4 leading-relaxed">
                En tant que <span class="font-bold text-emerald-700 underline decoration-2 italic">{{ userRole === 'admin' ? 'Administrateur' : 'Chef de Plateau' }}</span>, 
                ...
                <span class="text-red-500 font-bold italic text-[11px] mt-2 block bg-red-50 p-1 rounded">
                    <i class="pi pi-exclamation-circle mr-1"></i> ACTION IRRÉVERSIBLE : Verrouillage total de la saisie.
                </span>
            </p>
...
<style scoped>
:deep(.p-dialog-header) { padding: 1rem; font-size: 0.9rem; background: #f8fafc; border-bottom: 1px solid #e2e8f0; }
:deep(.p-dialog-content) { padding: 1.5rem 1rem; }
</style>
